...
class ExtensibilityTests(unittest.TestCase):
    ...
    def test_provenance_and_events_share_attempt_identity(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            status = (root / "run" / "public" / "status.html").read_text(encoding="utf-8")
            ...
            owner = (root / "run" / "status.html").read_text(encoding="utf-8")
            self.assertIn("PRIVATE", owner)
            self.assertIn("owner/attempts/", owner)
            self.assertTrue(
                (root / "run" / "owner" / "attempts" / "it001-i00-a00.html").is_file()
            )
